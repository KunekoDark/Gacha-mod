...
import com.gachamod.gacha.container.UpgradeTableContainer;
import com.gachamod.gacha.entitiys.tileentity.ModTileEntities;
import com.gachamod.gacha.entitiys.tileentity.UpgradeTableTile;
...
public class UpgradeTableBlock extends Block {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if(!worldIn.isRemote()){
            ...
            if(!player.isCrouching()){
                if(tileEntity instanceof UpgradeTableTile){
    ...
    private INamedContainerProvider createContainerProvider(World worldIn, BlockPos pos) {
        return new INamedContainerProvider() {
            ...
            public ITextComponent getDisplayName() {
                return new TranslationTextComponent("screen.gacha.upgrade_table");
            }

            @Nullable
            @Override
            public Container createMenu(int i, PlayerInventory playerInventory, PlayerEntity playerEntity) {
                return new UpgradeTableContainer(i , worldIn, pos, playerInventory, playerEntity);
            }
        };
    }
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return ModTileEntities.UPGRADE_TABLE_TILE.get().create();
    }
